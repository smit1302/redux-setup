...
const ViewMessage = () => {
    ...
    const handleNavigation = (route: any) => {
        history(route);
        dispatchData(addData({ key: "order_id", data: orderProductId.order_id }));
        dispatchData(addData({ key: "order_product_id", data: orderProductId.order_product_id }));
        dispatchData(addData({ key: "product_id", data: orderProductId.product_id }));
        dispatchData(addData({ key: "customer_id", data: orderProductId.customer_id }));

        console.log("ORDER_ID**", orderProductId.order_id);
        console.log("ORDER_PRODUCT_ID**", orderProductId.order_product_id);
        console.log("PRODUCT_ID", orderProductId.product_id);
        console.log("CUSTOMER_ID", orderProductId.customer_id);
    };
    ...
    const fetchMessageThread = async () => {
        try {
            const query = {
                order_product_id: orderProductId.order_product_id,
            };
            ...
            console.log("ORDER PRODUCT ID", orderProductId.order_product_id);
                    ...
                    <Grid item xs={12} md={6} lg={4}>
                        ...
                        <div style={{ marginBottom: '50px' }}>
                            <Card className='module_wrap'>
                                <MDBox className='module_head' variant="gradient" bgColor="info" borderRadius="lg" coloredShadow="success"
                                    mx={2} mt={-3} p={3} mb={1} textAlign="left" display="flex" justifyContent="space-between" alignItems="center">
                        ...
                        <div style={{ marginBottom: '50px' }}>
                            ...
                            <Card className='module_wrap'>
                                <MDBox className='module_head' variant="gradient" bgColor="info" borderRadius="lg" coloredShadow="success" mx={2} mt={-3} p={3} mb={1}
                        ...
                        <Grid container spacing={3}>
                            {note.map((noteItem: any, index: number) => (
                                <Grid item xs={12} key={index}>
                                    <Card style={{ height: '100%', width: '100%' }} className='module_wrap'>
                                        <MDBox p={2}>
                                            <MDBox className='module_head' variant="gradient" bgColor="info" borderRadius="lg"
                                                coloredShadow="success" mx={2} mt={-3} p={3} mb={1} textAlign="left" display="flex" justifyContent="space-between" alignItems="center">
                                                <MDTypography variant="h6" color="white" >
